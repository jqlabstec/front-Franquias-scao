...
function showSuccess(title = 'Sucesso', text = '') {
  return Swal.fire({ customClass:{popup:'vita',confirmButton:'vita-ok'}, icon:'success', title, text, confirmButtonText:'OK', buttonsStyling:false });
}
function showWarn(title = 'Atenção', text = '') {
  return Swal.fire({ customClass:{popup:'vita',confirmButton:'vita-warn'}, icon:'warning', title, text, confirmButtonText:'OK', buttonsStyling:false });
}
function showError(title = 'Erro', text = '') {
  return Swal.fire({ customClass:{popup:'vita',confirmButton:'vita-danger'}, icon:'error', title, text, confirmButtonText:'OK', buttonsStyling:false });
}
function toastSuccess(title = 'Salvo com sucesso') {
  return Swal.fire({ customClass:{popup:'vita'}, toast:true, position:'top-end', icon:'success', title, showConfirmButton:false, timer:1800, timerProgressBar:true });
}
function toastError(title = 'Falha na operação') {
  return Swal.fire({ customClass:{popup:'vita'}, toast:true, position:'top-end', icon:'error', title, showConfirmButton:false, timer:2200, timerProgressBar:true });
}
function confirmAdjust(title = 'Confirmar ajuste?', text = 'Deseja continuar?', danger = false) {
  ...
  return Swal.fire({
    customClass: { popup:'vita', confirmButton:'', cancelButton:'' },
...
function fmt(n, decimals = 3) {
  if (n == null) return '—';
  const x = Number(n);
  return Number.isNaN(x) ? '—' : x.toLocaleString(undefined, { minimumFractionDigits:0, maximumFractionDigits:decimals });
}
...
async function fetchTxns(q, token) {
  const p = new URLSearchParams({ page: String(q.page || 1), pageSize: String(PAGE_SIZE) });
...
async function fetchProducts(term, token) {
  const p = new URLSearchParams({ query: term, page:'1', pageSize:'50' });
...
async function fetchStock(productId, token) {
  const r = await fetch(`${API}/inventory/stock/${productId}`, {
    headers: { Authorization:`Bearer ${token}` }
  });
  const data = await r.json().catch(() => ({}));
  if (!r.ok) return null;
...
function renderTxns(items) {
  ...
  if (list.length === 0) {
    ...
    td.colSpan = 9; td.className = 'muted'; td.textContent = 'Sem resultados';  // ✅ colspan 9
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  let lastProducts = [];
  // ✅ category adicionado ao estado
  let txState = { page: 1, query: '', type: '', category: '' };
    ...
    searchTimer = setTimeout(async () => {
      try {
        ...
        for (const p of lastProducts) {
          ...
          // ✅ Badge de categoria no item do autocomplete
          const catStyle = p.category && CAT_COLORS[p.category]
            ? `background:${CAT_COLORS[p.category].bg};color:${CAT_COLORS[p.category].color};`
            : 'background:#f1f5f9;color:#475569;';
          const catHtml = p.category
            ? `<span class="item-cat" style="${catStyle}">${p.category}</span>`
            : '';

          el.innerHTML = `<span>${p.name}${p.code ? ` · <small>${p.code}</small>` : ''}</span>${catHtml}`;
  ...
  productList.addEventListener('click', async (e) => {
    ...
    productList.style.display = 'none';
    // ✅ Mostra categoria no hint
    const catLabel = p.category ? ` [${p.category}]` : '';
    productHint.textContent = `Selecionado: #${p.id} · ${p.name}${catLabel}`;
    const stock = await fetchStock(p.id, auth.token).catch(() => null);
    stockHint.textContent = stock?.currentQty != null ? `Saldo atual: ${fmt(stock.currentQty, 3)}` : 'Sem saldo registrado ainda.';
    costHint.textContent  = stock?.avgUnitCost != null ? `Custo médio atual: ${Number(stock.avgUnitCost).toFixed(6)}` : '';
    qty.focus();
  });
  ...
  form.addEventListener('submit', async (ev) => {
    ...
    try {
      ...
      if (stock?.currentQty  != null) stockHint.textContent = `Saldo atual: ${fmt(stock.currentQty, 3)}`;
      if (stock?.avgUnitCost != null) costHint.textContent  = `Custo médio atual: ${Number(stock.avgUnitCost).toFixed(6)}`;
    } catch (e) {
  ...
  btnClear.addEventListener('click', () => {
    productId.value = ''; productSearch.value = '';
    productHint.textContent = 'Selecione um produto na lista.';
    qty.value = ''; unitCost.value = ''; notes.value = '';
    stockHint.textContent = ''; costHint.textContent = '';
  });
  ...
  document.getElementById('txBody').addEventListener('click', async (e) => {
    ...
    stockHint.textContent = stock?.currentQty  != null ? `Saldo atual: ${fmt(stock.currentQty, 3)}` : '';
    costHint.textContent  = stock?.avgUnitCost != null ? `Custo médio atual: ${Number(stock.avgUnitCost).toFixed(6)}` : '';
    qty.focus();
    toastSuccess('Formulário preenchido. Ajuste e confirme.');
  });

  /* ── Filtros do histórico — ✅ inclui category ── */
  document.getElementById('txSearch').addEventListener('click', async () => {
    txState = {
      page:     1,
      query:    document.getElementById('txQuery').value.trim(),
  ...
  txClear.addEventListener('click', async () => {
    ...
    txState = { page: 1, query: '', type: '', category: '' };
    await loadTxns();
  });

  txPrevBtn.addEventListener('click', async () => {
    if (txState.page > 1) { txState.page--; await loadTxns(); }
  });
  txNextBtn.addEventListener('click', async () => {
    txState.page++; await loadTxns();
  });
