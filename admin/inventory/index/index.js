...
function buildQuery(state){
  ...
  if (state.belowMin) p.set('onlyBelowMin', 'true');
  return p.toString();
}
...
function renderRows(items){
  ...
  for (const it of items){
    ...
    tr.innerHTML = `
      ...
      <td>${fmt(it.currentQty, 3)}</td>
      <td>${fmtMoney(it.avgUnitCost)}</td>
      <td>${fmt(it.minQty, 3)}</td>
...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  const belowMin = document.getElementById('belowMin');
  const prevBtn = document.getElementById('prevBtn');
  const nextBtn = document.getElementById('nextBtn');
  const pageInfo = document.getElementById('pageInfo');
  const clearBtn = document.getElementById('clearBtn');

  let state = { q:'', belowMin:false, page:1 };
  ...
  document.getElementById('filterForm').addEventListener('submit', (ev)=>{
    ev.preventDefault();
    state.q = q.value.trim(); state.belowMin = !!belowMin.checked; state.page = 1; load();
  });
  clearBtn.addEventListener('click', ()=>{ q.value=''; belowMin.checked=false; state={ q:'', belowMin:false, page:1 }; load(); });
